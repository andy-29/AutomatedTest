...
@ddt
class Live_Get_Services(unittest.TestCase):
    '''
    直播中获取美购相关信息
    '''
    ...
    def setUpClass(cls):
        ...
        #获取医院id
        url = self.host + g.get_info('api_info','search_v2_hospital')+ "?" + self.android_params
        r = gmhttp.get(url,verify=False).json()
        self.hospital_list = [item['hospital_id'] for item in r['data']['hospitals']]
    ...
    def test_live_get_services(self,value):
        ...
        for hospital in self.hospital_list:
            url = self.host + self.api_name+ "?" + self.android_params +'&hospital_id={}&q=眼'.format(hospital)
            r = gmhttp.get(url,verify=False)
            self.assertEqual(r.status_code,200,'返回码不为200！')
            r = r.json()
            self.assertEqual(r.get("error"),0)
            self.assertIn('data',r.keys())
            self.assertIsInstance(r.get('data'),(dict,list))
            if r.get('data') and isinstance(r.get('data'),list):
