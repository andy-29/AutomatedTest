...
@ddt
class Video_List(unittest.TestCase):
    '''
    小视频列表
    '''
    ...
    def test_video_list(self,value):
        ...
        for item in r.get('data'):
            ...
            self.assertIn('feed_id', item.keys())
            self.assertIsInstance(item.get('feed_id'), str)
            self.assertIn('city', item.keys())
            self.assertIsInstance(item.get('city'), (str,type(None)))
            ...
            self.assertIn('item_type', item.keys())
            self.assertIsInstance(item.get('item_type'), str)
